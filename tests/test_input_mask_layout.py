...
def test_input_appends_mask_and_count(interpreter4):
    # 2D, S=4, K=3 neighbors
    S, K = 4, 3
    cells = [
        Cell([0.0, 0.0], PassthroughGenome(), state_size=S, interpreter=interpreter4, max_neighbors=K),
        Cell([1.0, 0.0], PassthroughGenome(), state_size=S, interpreter=interpreter4, max_neighbors=K),
        Cell([2.0, 0.0], PassthroughGenome(), state_size=S, interpreter=interpreter4, max_neighbors=K),
    ]
    w = World(cells)
